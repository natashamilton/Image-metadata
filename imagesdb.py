...
class Images:
    def __init__(self, image_title, image_id, filename, img_extension, img_size, image_owner, licence_type,):
...
class GUI:
    def __init__(self):
        ...
        img_size_label = Label(window, text='Enter Image Size:')
        img_size_label.pack()
...
#Attribution-NonCommercial-NoDerivs - Free to copy and redistribute non-commercially, but must give credit
    ...
    def doSubmit(self):
        ...
        if noduplicate == True: #test for making sure all entry fields are filled
            if len(self.image_title_field.get()) <1 or len(self.image_owner_field.get()) <1 or len(self.image_id_field.get()) <1 or len(self.img_extension_field.get()) <1 or len(self.filename_field.get()) <1 or len(self.img_size_field.get()) <1 or len(self.licence_type_field.get()) <1:
                tkinter.messagebox.showwarning('Warning!','Please enter a value for all fields')
            else:
                try:
                    validated_img_size = int(self.img_size_field.get())#make into int and store as 'validated' variable
                    validated_image_id = int(self.image_id_field.get())#make into int and store as 'validated' variable
                    self.recordlist.append(Images(self.image_title_field.get(), self.image_owner_field.get(), self.image_id_field.get(),  self.filename_field.get(), self.img_size_field.get(), self.licence_type_field.get(), self.img_extension_field()))
    ...
    def doClear(self):#command to clear field
    ...
    def writetocsv(self):
        ...
        if self.ready_to_write: #cheacks data has been previously validated
            ...
            for record in self.recordlist:
                print(record.get_book_title())
                writer.writerow([record.get_image_title(), record.get_image_owner(), record.image_id_published(), record.get_filename(), record.get_img_size(), record.get_licence_type()])
